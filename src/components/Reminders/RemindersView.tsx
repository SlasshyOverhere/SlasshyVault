...
export function RemindersView() {
  ...
  const [activeTab, setActiveTab] = useState<'discover' | 'reminders'>('discover')
  ...
  useEffect(() => {
    ...
    let unlistenRefresh: any
    const setup = async () => {
      const { listen } = await import('@tauri-apps/api/event')
      unlistenRefresh = await listen('refresh-reminders', () => {
        loadReminders()
      })
    }
    setup()
  ...
  const handleToggleNotifications = async (enabled: boolean) => {
    ...
    try {
      ...
      toast({
        ...
        description: enabled 
          ? "You will receive native alerts for your reminders." 
          : "Reminders will still be tracked but alerts are silenced."
      })
    } catch (error) {
  ...
  const handleSetReminderFromSearch = (data: any) => {
          ...
          <div className="relative group">
            ...
            >
              <div className="relative">
                ...
                <div className="w-12 h-12 rounded-[1.25rem] bg-white/5 border border-white/10 flex items-center justify-center shadow-elevation-1 group-hover:border-white/20 transition-all duration-500">
                  <Clapperboard className="w-6 h-6 text-white/70 group-hover:text-white transition-colors" />
                </div>
              </div>
          ...
          <div className="flex p-0.5 rounded-full bg-card/90 backdrop-blur-xl border border-white/10 shadow-md">
            <button
              ...
              className={cn(
                ...
                activeTab === 'discover' ? "text-black" : "text-muted-foreground hover:text-foreground"
              )}
              tabIndex={0}
            >
              {activeTab === 'discover' && (
                <motion.div
                  layoutId="RemindersTab"
            ...
            <button
              ...
              className={cn(
                "relative flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all duration-200",
                activeTab === 'reminders' ? "text-black" : "text-muted-foreground hover:text-foreground"
              )}
              tabIndex={0}
            >
              {activeTab === 'reminders' && (
        ...
        >
           ...
           <div className="flex items-center gap-4 px-4 py-2 bg-white/[0.02] border border-white/[0.05] rounded-xl backdrop-blur-sm">
            <div className="flex flex-col items-end">
              ...
              <div className="flex items-center gap-1.5">
                <div className={cn(
                  ...
                  config?.notifications_enabled ? "bg-white shadow-glow-sm animate-pulse" : "bg-white/10"
                )} />
                <span className={cn(
                  "text-[9px] font-black uppercase tracking-widest leading-none transition-colors duration-500",
                  config?.notifications_enabled ? "text-white/80" : "text-white/30"
                )}>
            ...
            <Switch 
              ...
              className="scale-75 data-[state=checked]:bg-white transition-colors"
            />
          </div>
        </motion.div>
      </header>
          ...
          <AnimatePresence mode="wait">
            {activeTab === 'discover' ? (
              ...
              >
                <ScrollArea className="w-full flex-1 px-6 pb-12 [&>div]:scrollbar-none">
                  <div className="w-full space-y-12 pt-4 flex flex-col items-center">
                    ...
                    {/* Hero Search Section - Centered Content */}
                    <div className={cn(
                      "w-full relative group/search pt-4 max-w-3xl transition-all duration-700",
                      searchResults.length > 0 ? "mt-0" : "mt-32"
                    )}>
                      ...
                      <div className="space-y-6">
                        <div className="flex flex-col gap-4">
                          ...
                          <Button 
                            ...
                            className="h-16 px-12 rounded-[1.5rem] bg-white text-black hover:bg-white/90 font-black uppercase tracking-[0.25em] text-[11px] shadow-glow-sm active:scale-95 transition-all duration-500 w-full"
                          >
                            Search
                          </Button>
                        </div>
                            ...
                            <div className="flex flex-wrap justify-center gap-x-4 gap-y-2">
                              {trendingSuggestions.map(item => (
                                <button
                                  ...
                                  onClick={() => openTrendingDetails(item)}
                                  className="text-[10px] font-bold hover:text-white transition-colors hover:underline underline-offset-4 decoration-white/20"
                                >
                                  {item.title}
                                </button>
                              ))}
                          ...
                          <div className="flex w-fit items-center gap-1.5 rounded-2xl p-1.5 bg-white/[0.03] border border-white/[0.05] backdrop-blur-md">
                            {[
                              ...
                              <button 
                                ...
                                className={cn(
                                  "px-5 py-2 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] transition-all duration-500", 
                                  mediaFilter === tab.id 
                          ...
                          {filteredResults.map((result, idx) => (
                            ...
                            >
                              ...
                              >
                                <div className="relative overflow-hidden rounded-[2rem] border border-white/[0.06] bg-white/[0.02] shadow-elevation-1 transition-all duration-500 group-hover:border-white/15 group-hover:shadow-elevation-2">
                                  ...
                                  <div className="absolute top-4 left-4 z-20">
                                    <div className="h-7 px-3 flex items-center gap-2 rounded-full border border-white/10 bg-black/35 backdrop-blur-xl transition-colors group-hover:border-white/30">
                                  ...
                                  <div className="pointer-events-none absolute inset-0 z-30 flex flex-col justify-end p-6 opacity-0 transition-all duration-500 translate-y-4 group-hover:opacity-100 group-hover:translate-y-0 group-hover:pointer-events-auto">
                                    <div className="flex gap-2">
                                      <Button
                                        size="sm"
                                        variant="outline"
                                        className="w-full h-11 rounded-2xl border-white/15 bg-black/40 text-white hover:bg-white/10 hover:text-white"
                                        onClick={(e) => {
                                          e.stopPropagation()
                                          handleSetReminderFromSearch({
                              ...
                              <div className="mt-5 px-1 space-y-1.5 text-center">
                                <h3 className="font-black text-white text-sm line-clamp-1 group-hover:text-white transition-colors tracking-tight">
              ...
              <motion.div 
                ...
                exit={{ opacity: 0, y: -30 }}
                transition={{ duration: 0.6, ease: [0.22, 1, 0.36, 1] }}
                className="flex flex-col h-full min-h-0 overflow-hidden px-6 pb-8 items-center"
              >
